...
for ((i=0; i<=iter_id; i++)); do
    data_dirs+="data/ablation_self_correct/alfworld/sft/iter${i},"
done
...
if [ "$iter_id" -eq 0 ]; then
    ...
    model_id="leap-llm/Meta-Llama-3-8B-Instruct-sft-self-correct-alfworld-iter${prev_iter_id}"
fi
...
python scripts/train/sft.py \
    --data_dirs "${data_dirs}" \
    --output_dir save/${current_date}/alfworld_sft_self_correct/iter${iter_id} \
    --model_id ${model_id} \

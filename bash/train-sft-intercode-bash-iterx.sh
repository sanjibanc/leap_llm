...
for ((i=0; i<=iter_id; i++)); do
    data_dirs+="data/intercode_bash/sft/iter${i},"
done
...
if [ "$iter_id" -eq 0 ]; then
    ...
    model_id="leap-llm/Meta-Llama-3.1-70B-Instruct-sft-intercode-bash-iter${prev_iter_id}"
fi
...
python scripts/train/sft.py \
    --data_dirs "${data_dirs}" \
    --output_dir save/${current_date}/intercode_bash/sft/iter${iter_id} \
    --model_id ${model_id} \

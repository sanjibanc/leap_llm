...
for ((i=0; i<=iter_id; i++)); do
    data_dirs+="data/ablation_self_correct/webshop/sft/iter${i},"
done
...
elif [ "$iter_id" -eq 1 ]; then
    model_id="leap-llm/Meta-Llama-3-8B-Instruct-sft-webshop-iter0"
else
    model_id="leap-llm/Meta-Llama-3-8B-Instruct-sft-self-correct-webshop-iter${prev_iter_id}"
fi
...
python scripts/train/sft.py \
    --data_dirs "${data_dirs}" \
    --output_dir save/${current_date}/webshop_sft_self_correct/iter${iter_id} \
    --model_id ${model_id} \
